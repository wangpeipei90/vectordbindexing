...
class HNSWIndex:
    ...
    def search_steps_to_target(self, vector: np.ndarray, target_id: int, k: int = 10, ef: Optional[int] = None, 
        ...
        if enhanced_layer1_search and self.max_level >= 1:
            if multi_path_search:
                if verbose:
                    print(f"第1层多路并发搜索: 使用beam width {layer1_eff}, 最大路径数 {max_paths}")
                ...
                if verbose:
                    print(f"第1层多路搜索完成: 步数 {len(steps)}, 使用加速边: {accel_edges}, 找到目标: {found}, 探索路径数: {paths_explored}")
            else:
                if verbose:
                ...
                if verbose:
                    print(f"第1层搜索完成: 步数 {len(steps)}, 使用加速边: {accel_edges}, 找到目标: {found}")
    ...
    def _search_layer_multi_path(self, vec: np.ndarray, entry_id: int, layer: int, ef: int, 
        ...
        if verbose:
            print(f"多路并发搜索起始点: {entry_id}, 距离: {dist_entry:.6f}, 最大路径数: {max_paths}")
        ...
        while candidates and not found:
            ...
            for i, (d, nb) in enumerate(neighbors_to_explore):
                ...
                if verbose:
                    edge_type = "加速边" if is_accel_edge else "普通边"
                    print(f"多路访问节点: {nb}, 距离: {d:.6f}, 路径: {i+1}, 边类型: {edge_type}")
                ...
                if nb == target_id:
                    found = True
                    if verbose:
                        print(f"多路搜索找到目标节点: {target_id}, 距离: {d:.6f}, 路径: {i+1}")
                    break
        ...
        if verbose:
            print(f"多路并发搜索完成: 步数 {len(steps)}, 加速边 {accel_edges}, 找到目标: {found}, 探索路径数: {paths_explored}")
