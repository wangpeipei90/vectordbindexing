...
class DataPreprocessor:
    ...
    def fit(self, text_data: np.ndarray, image_data: np.ndarray,
        ...
        if sample_size is not None:
            ...
            text_indices = np.random.choice(
                len(text_data), n_text, replace=False)
            image_indices = np.random.choice(
                len(image_data), n_image, replace=False)
        ...
        print(
            f"Fitting preprocessor on {len(text_sample)} text samples and {len(image_sample)} image samples")
    ...
    def _calculate_sub_modality_scaling(self, text_data: np.ndarray, image_data: np.ndarray):
        ...
        print(
            f"Sub-modality scaling factors - Text: {self.text_scaling_factor:.4f}, Image: {self.image_scaling_factor:.4f}")
...
class HNSWIndex:
    ...
    def add_item(self, vector: np.ndarray, id: Optional[int] = None,
        ...
        self.items[id] = Node(vector=vec, level=level,
                              modality=modality, original_id=original_id)
    ...
    def add_item_fast10k(self, vector: np.ndarray, id: Optional[int] = None,
                         lsh: Optional[object] = None, limit: int = 10000) -> int:
        """
        快速添加项目的方法，与其他HNSW实现保持一致
        ...
        int
            分配的项目ID
        """
        return self.add_item(vector, id)
    ...
    def add_items_batch(self, vectors: np.ndarray, ids: Optional[List[int]] = None,
        ...
        if preprocessed:
            ...
            for i, (vec, modality) in enumerate(zip(vectors, modalities)):
                processed_vec = self.preprocessor.transform_single(
                    vec, modality)
        ...
        for start_idx in range(0, n_vectors, batch_size):
            ...
            for i, (vec, id_val, modality, orig_id) in enumerate(zip(batch_vectors, batch_ids, batch_modalities, batch_original_ids)):
                level = self._assign_level()
                self.items[id_val] = Node(
                    vector=vec, level=level, modality=modality, original_id=orig_id)
    ...
    def _search_layer_greedy_trace(self, vec: np.ndarray, entry_id: int, layer: int) -> Tuple[int, List[int], bool]:
        """Greedy search for the closest neighbour on a specific layer with trace."""
        current = entry_id
        best_dist = self.distance(vec, self.items[current].vector)
        improved = True
        trace = [current]
    ...
    def query_with_distances(self, vector: np.ndarray, k: int, modality: str = "unknown") -> List[Tuple[int, float]]:
...
def create_preprocessed_index(text_data_path: str, image_data_path: str,
    ...
    print(
        f"Loaded {len(text_data)} text vectors and {len(image_data)} image vectors")
    ...
    for i, vec in enumerate(image_data):
        index.add_item(vec, id=len(text_data) + i,
                       modality="image", original_id=i)
        if (i + 1) % 10000 == 0:
    ...
    for modality, stats in modality_stats.items():
        print(
            f"{modality}: {stats['count']} vectors, avg level: {stats['avg_level']:.2f}")
